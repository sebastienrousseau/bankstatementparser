...
class Pain001Parser(BankStatementParser):
    ...
    def parse(self, output_file: Optional[str] = None, redact_pii: bool = False) -> pd.DataFrame:
        ...
        try:
            ...
            if group_header is not None:
                ...
                header_fields['InitgPty'] = init_party_elem.text if init_party_elem is not None else None
            ...
            for pmt in payment_info_records:
                ...
                for child in pmt:
                    if child.tag in ['PmtInfId', 'PmtMtd', 'NbOfTxs', 'CtrlSum', 'ReqdExctnDt', 'ChrgBr']:
                        ...
                        pmt_fields['DbtrNm'] = dbtr_name.text if dbtr_name is not None else None
                        # Extract debtor account
                        dbtr_acct = child.find('DbtrAcct/Id/IBAN')
                        pmt_fields['DbtrIBAN'] = dbtr_acct.text if dbtr_acct is not None else None
                    elif child.tag == 'DbtrAgt':
                        # Extract debtor agent
                        dbtr_agt = child.find('FinInstnId/BIC')
                        pmt_fields['DbtrBIC'] = dbtr_agt.text if dbtr_agt is not None else None

                # Batch process all transactions for this payment
                transactions = pmt.findall('CdtTrfTxInf')
                for tx in transactions:
                    payment = pmt_fields.copy()  # Start with payment-level data

                    # Pre-extract all transaction fields in single iteration
                    for child in tx:
                        if child.tag == 'PmtId':
                            end_to_end_elem = child.find('EndToEndId')
                            payment['EndToEndId'] = end_to_end_elem.text if end_to_end_elem is not None else None
                        elif child.tag == 'Amt':
                            ...
                            payment['CdtrBIC'] = cdtr_agt_elem.text if cdtr_agt_elem is not None else None
                        elif child.tag == 'Cdtr':
                            cdtr_name_elem = child.find('Nm')
                            payment['CdtrNm'] = cdtr_name_elem.text if cdtr_name_elem is not None else None
                        elif child.tag == 'RmtInf':
                            ustrd_elem = child.find('Ustrd')
                            payment['RmtInf'] = ustrd_elem.text if ustrd_elem is not None else None
    ...
    def parse_streaming(self, redact_pii: bool = False) -> Generator[Dict[str, Any], None, None]:
        ...
        if isinstance(self.file_name, str):
            validator = InputValidator()
            try:
                validated_path = validator.validate_input_file_path(self.file_name)
                file_path = str(validated_path)
                logger.info(f"Input file validated for streaming: {file_path}")
            except (ValidationError, FileNotFoundError) as e:
                logger.error(f"File validation failed for streaming {self.file_name}: {e}")
                raise
        else:
                ...
                if event == 'end' and elem.tag == 'GrpHdr':
                    ...
                    for child in elem:
                        if child.tag in ['MsgId', 'CreDtTm', 'NbOfTxs']:
                            ...
                            header_fields['InitgPty'] = nm_elem.text if nm_elem is not None else None
                    ...
                    try:
                        payment_data = self._parse_streaming_payment(elem, current_payment_info, header_fields, redact_pii)
                        yield payment_data
                    except Exception as e:
                        logger.warning(f"Error parsing payment transaction: {e}")
    ...
    def get_summary(self, redact_pii: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            group_header = root.find('.//CstmrCdtTrfInitn/GrpHdr')
            header_data = {'MsgId': 'Unknown', 'CreDtTm': 'Unknown', 'NbOfTxs': '0', 'InitgPty': 'Unknown'}

            if group_header is not None:
                # Batch extract all header fields in one iteration
                for child in group_header:
                    if child.tag in ['MsgId', 'CreDtTm', 'NbOfTxs']:
                        header_data[child.tag] = child.text if child.text else 'Unknown'
                    elif child.tag == 'InitgPty':
                        nm_elem = child.find('Nm')
                        header_data['InitgPty'] = nm_elem.text if nm_elem is not None and nm_elem.text else 'Unknown'
            ...
            return {
                ...
                'transaction_count': int(header_data['NbOfTxs']) if header_data['NbOfTxs'].isdigit() else 0,
