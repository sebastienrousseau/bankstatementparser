...
**Yes — byte-identical output on every run for the deterministic path.** Given the same input file, `CamtParser`, `Pain001Parser`, `CsvStatementParser`, `OfxParser`, `Mt940Parser`, and `QfxParser` produce the same result every time. No randomness, no model inference, no heuristic sampling. Verify this yourself: run the same file twice and diff the output. CI enforces determinism with 697 tests at 100% branch coverage, including property-based fuzzing via Hypothesis.
...
```bash
poetry run pytest                             # 697 tests, 100% branch coverage
