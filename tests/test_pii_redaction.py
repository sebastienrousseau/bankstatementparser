...
class TestPIIRedaction(unittest.TestCase):
    ...
    def test_cli_default_console_output_redacts_pii(self, mock_camt_parser):
        ...
        mock_parser_instance.get_statement_stats.return_value = (
            self.test_data_with_pii.to_dict("records")
        )
    ...
    @patch("bankstatementparser.cli.CamtParser")
    def test_cli_show_pii_flag_displays_unredacted_with_warning(
    ...
    def test_cli_output_file_export_unredacted(
    ...
    @patch("pandas.DataFrame.to_csv")
    def test_cli_pain001_output_file_export_unredacted(
    ...
    def test_edge_case_partial_keyword_matching(self):
        ...
        self.assertEqual(
            redacted_df["account_holder"].iloc[0], "***REDACTED***"
        )
