...
class TestBankStatementCLI(unittest.TestCase):
    ...
    def test_parse_camt_success_with_output(self, mock_to_csv, mock_camt_parser):
        """Test successful CAMT parsing with output file."""
        # Setup mocks
        mock_parser_instance = Mock()
        mock_parser_instance.get_statement_stats.return_value = [{"test": "data"}]
    ...
    @patch('pandas.DataFrame.to_csv')
    def test_parse_pain_success_with_output(self, mock_to_csv, mock_pain_parser):
        ...
        with patch.object(self.cli, '_sanitize_file_path', return_value='/path/test.xml'):
            with patch.object(self.cli.validator, 'validate_input_file_path', return_value=Path('/path/test.xml')):
                with patch.object(self.cli, 'parse_camt') as mock_parse:
                    self.cli.run()
                    mock_parse.assert_called_once_with(Path('/path/test.xml'), None, False)

    @patch('sys.argv', ['cli.py', '--type', 'pain001', '--input', 'test.xml'])
    @patch('bankstatementparser.cli.logger')
    ...
    @patch('sys.argv', ['cli.py', '--type', 'camt', '--input', 'test.xml', '--output', 'output.csv'])
    @patch('bankstatementparser.cli.logger')
    def test_run_with_output_success(self, mock_logger):
        """Test successful parsing with output file via run method."""
        with patch.object(self.cli, '_sanitize_file_path', side_effect=['/path/test.xml', '/path/output.csv']):
            with patch.object(self.cli.validator, 'validate_input_file_path', return_value=Path('/path/test.xml')):
                with patch.object(self.cli.validator, 'validate_output_file_path', return_value=Path('/path/output.csv')):
                    with patch.object(self.cli, 'parse_camt') as mock_parse:
                        self.cli.run()
                        mock_parse.assert_called_with(Path('/path/test.xml'), Path('/path/output.csv'), False)

    @patch('sys.argv', ['cli.py', '--type', 'camt', '--input', 'test.xml', '--max-size', '50'])
    @patch('bankstatementparser.cli.logger')
        ...
        with patch.object(self.cli, '_sanitize_file_path', return_value='/path/test.xml'):
            with patch.object(self.cli.validator, 'validate_input_file_path', return_value=Path('/path/test.xml')):
                with patch.object(self.cli, 'parse_camt') as mock_parse:
                    ...
                    self.assertEqual(self.cli.validator.max_file_size, 50 * 1024 * 1024)
                    mock_parse.assert_called_once()

    @patch('sys.argv', ['cli.py', '--type', 'camt', '--input', 'invalid.xml'])
    def test_run_input_validation_failed(self):
        """Test run with input validation failure."""
        with patch.object(self.cli, '_sanitize_file_path', side_effect=ValidationError("Invalid path")):
            with patch('sys.exit') as mock_exit:
                with patch('builtins.print') as mock_print:
                    self.cli.run()
                    mock_exit.assert_called_with(1)
    ...
    @patch('sys.argv', ['cli.py', '--type', 'camt', '--input', 'test.xml', '--output', 'invalid.csv'])
    def test_run_output_validation_failed(self):
        """Test run with output validation failure."""
        with patch.object(self.cli, '_sanitize_file_path', side_effect=['/path/test.xml', '/path/invalid.csv']):
            with patch.object(self.cli.validator, 'validate_input_file_path', return_value=Path('/path/test.xml')):
                with patch.object(self.cli.validator, 'validate_output_file_path', side_effect=ValidationError("Invalid output")):
                    with patch('sys.exit') as mock_exit:
                        with patch('builtins.print') as mock_print:
                            self.cli.run()
                            mock_exit.assert_called_with(1)
    ...
    @patch('sys.argv', ['cli.py', '--type', 'unsupported', '--input', 'test.xml'])
    ...
    def test_run_parsing_exception(self):
        ...
        with patch.object(self.cli, '_sanitize_file_path', return_value='/path/test.xml'):
            with patch.object(self.cli.validator, 'validate_input_file_path', return_value=Path('/path/test.xml')):
                with patch.object(self.cli, 'parse_camt', side_effect=Exception("Parse error")):
                    with patch('sys.exit') as mock_exit:
                        with patch('builtins.print') as mock_print:
                            self.cli.run()
                            mock_exit.assert_called_with(1)
