...
class TestPain001Parser(unittest.TestCase):
    ...
    def testMessageIdentification(self):
        ...
        message_id = (
            group_header.find("MsgId").text
            if group_header.find("MsgId") is not None
            else None
        )
        self.assertEqual(
            message_id,
            "1",
            "Message identification not retrieved correctly",
        )
    ...
    def testCreationDateTime(self):
        ...
        group_header = self.parser.tree.find(".//GrpHdr")
        creation_date_time = (
            group_header.find("CreDtTm").text
            if group_header.find("CreDtTm") is not None
            else None
        )
        self.assertEqual(
            creation_date_time,
    ...
    def testNumberOfTransactions(self):
        ...
        number_of_transactions = (
            group_header.find("NbOfTxs").text
            if group_header.find("NbOfTxs") is not None
            else None
        )
        self.assertEqual(
            number_of_transactions,
    ...
    def testInitiatingParty(self):
        ...
        initiating_party = (
            self.parser.tree.find(".//InitgPty/Nm").text
            if self.parser.tree.find(".//InitgPty/Nm") is not None
            else None
        )
        self.assertEqual(
            initiating_party,
    ...
    def test_parse_exception_handling(self):
        ...
        try:
            ...
            with self.assertRaises(ParseError) as context:
                parser.parse()
            self.assertIn(
                "Error parsing PAIN.001 file", str(context.exception)
            )
        finally:
            os.unlink(temp_path)
