...
class EuxisDispatchTestSuite:
            ...
            try:
                ...
                if missing_fields:
                    ...
                    validation_results.append({
                        "file": manifest_file.name,
                        "status": "PASSED",
                        "tasks": len(manifest_data.get("dispatches", []))
                    })
    ...
    def _generate_recommendations(self, results: list[dict[str, Any]]) -> list[str]:
        ...
        if not failed_results:
            recommendations.append("✅ All euxis-dispatch tests passed successfully!")
            recommendations.append("✅ The dispatch system appears to be working correctly.")
            recommendations.append("✅ Consider running stress tests for production readiness.")
        else:
            recommendations.append("❌ Some tests failed. Review the following:")

            for failed_result in failed_results:
                test_type = failed_result["test_type"]
                recommendations.append(f"❌ {test_type} tests failed - investigate and fix")

            if any(r["test_type"] == "basic_functionality" for r in failed_results):
                recommendations.append("🔥 CRITICAL: Basic functionality tests failed - core dispatch system needs fixes")

            if any(r["test_type"] == "edge_cases" for r in failed_results):
                recommendations.append("⚠️ Edge case handling needs improvement for robustness")

            if any(r["test_type"] == "performance" for r in failed_results):
                recommendations.append("⚠️ Performance issues detected - optimize dispatch algorithms")
    ...
    def print_test_report(self, report: dict[str, Any]) -> None:
        ...
        for result in report["detailed_results"]:
            ...
            print(f"{status_icon} {result['test_type']}: {result['status']}")

            if result.get("error"):
                print(f"   Error: {result['error']}")

            if result.get("results") and result["test_type"] == "manifest_validation":
                print(f"   Manifests tested: {len(result['results'])}")
                for manifest_result in result["results"]:
                    icon = "✅" if manifest_result["status"] == "PASSED" else "❌"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run euxis-dispatch integration tests")
    ...
    parser.add_argument("--json-output", help="Save report as JSON to specified file")
