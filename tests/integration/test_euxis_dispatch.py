...
class MockEuxisDispatcher:
    ...
    def _group_by_stage(
        ...
        for task in tasks:
            stage = task.get(
                "stage", 0
            )  # Default to stage 0 for backward compatibility
    ...
    def _task_completed(
        self, agent_id: str, completed_tasks: list[dict[str, Any]]
    ) -> bool:
        """Check if a task has been completed."""
        return any(
            task["agent"] == agent_id for task in completed_tasks
        )

    def _depends_on_failed_task(
        self, task: dict[str, Any], failed_agent: str
    ) -> bool:
    ...
    def _execute_task(self, task: dict[str, Any]) -> dict[str, Any]:
        ...
        if conflicted_locks:
            return {
                "agent": agent,
                "status": "FAILED",
                "reason": f"Lock conflict: {conflicted_locks[0]} already held",
            }
...
class TestEuxisDispatchIntegration:
    ...
    def test_stage_based_execution_ordering(self, temp_dir, dispatcher):
        ...
        manifest_path = self.create_manifest(
            temp_dir, "stage_order_test.json", manifest_data
        )
        ...
        executed_agents = [
            task["agent"] for task in dispatcher.executed_tasks
        ]

        stage1_indices = [
            executed_agents.index(agent) for agent in stage1_agents
        ]
        stage2_indices = [
            executed_agents.index(agent) for agent in stage2_agents
        ]

        assert max(stage1_indices) < min(stage2_indices)
        assert result["status"] == "SUCCESS"

    def test_depends_on_dependency_resolution(
        self, temp_dir, dispatcher
    ):
        ...
        manifest_path = self.create_manifest(
            temp_dir, "dependency_test.json", manifest_data
        )
        result = dispatcher.execute_manifest(str(manifest_path))
    ...
    def test_file_lock_acquisition_and_release(
        self, temp_dir, dispatcher
    ):
        ...
        manifest_path = self.create_manifest(
            temp_dir, "locking_test.json", manifest_data
        )
        result = dispatcher.execute_manifest(str(manifest_path))
        ...
        executed_tasks = {
            task["agent"]: task["locks"]
            for task in dispatcher.executed_tasks
        }
        assert executed_tasks["file-processor-1"] == [
            "database.db",
            "config.json",
        ]
    ...
    def test_lock_conflict_prevention(self, temp_dir, dispatcher):
        ...
        manifest_path = self.create_manifest(
            temp_dir, "lock_conflict_test.json", manifest_data
        )
        result = dispatcher.execute_manifest(str(manifest_path))
        ...
        failed_results = [
            r for r in result["results"] if r["status"] == "FAILED"
        ]
        assert len(failed_results) >= 1
        assert "lock" in failed_results[0]["reason"].lower()

    def test_backward_compatibility_legacy_manifests(
        self, temp_dir, dispatcher
    ):
        ...
        manifest_path = self.create_manifest(
            temp_dir, "legacy_test.json", legacy_manifest
        )
        result = dispatcher.execute_manifest(str(manifest_path))
    ...
    def test_stage_failure_abort_behavior(self, temp_dir, dispatcher):
        ...
        manifest_path = self.create_manifest(
            temp_dir, "failure_abort_test.json", manifest_data
        )
        result = dispatcher.execute_manifest(str(manifest_path))
    ...
    def test_dependency_failure_propagation(self, temp_dir, dispatcher):
        ...
        manifest_path = self.create_manifest(
            temp_dir, "dependency_failure_test.json", manifest_data
        )
        result = dispatcher.execute_manifest(str(manifest_path))
        ...
        skipped_results = [
            r for r in result["results"] if r["status"] == "SKIPPED"
        ]
        if skipped_results:
            skipped_agents = [r["agent"] for r in skipped_results]
            assert "dependent-task" in skipped_agents

    def test_mixed_legacy_and_modern_manifests(
        self, temp_dir, dispatcher
    ):
        ...
        manifest_path = self.create_manifest(
            temp_dir, "mixed_test.json", mixed_manifest
        )
        result = dispatcher.execute_manifest(str(manifest_path))
    ...
    def test_concurrent_execution_simulation(
        ...
        manifest_path = self.create_manifest(
            temp_dir, "concurrent_test.json", manifest_data
        )
        result = dispatcher.execute_manifest(str(manifest_path))
        ...
        manifest_data = {
            ...
            "dispatches": [
                ...
                # Final layer - depends on third layer
                {
                    ...
                    "task": "Final",
                    "verify_cmd": "echo ok",
                    "depends_on": ["layer3a", "layer3b"],
                },
            ],
        }

        manifest_path = self.create_manifest(
            temp_dir, "complex_deps_test.json", manifest_data
        )
        result = dispatcher.execute_manifest(str(manifest_path))
