...
class TestEuxisDispatchPerformance:
    ...
    def test_large_manifest_performance(self, temp_dir, dispatcher):
        ...
        manifest_path = self.create_manifest(
            temp_dir, "large_performance.json", manifest_data
        )
    ...
    def test_deep_dependency_chain_performance(
        ...
        manifest_path = self.create_manifest(
            temp_dir, "deep_chain_perf.json", manifest_data
        )
        ...
        print(
            f"Deep dependency chain execution time: {execution_time:.3f}s"
        )

    def test_wide_dependency_fan_out_performance(
        self, temp_dir, dispatcher
    ):
        ...
        manifest_path = self.create_manifest(
            temp_dir, "wide_fanout.json", manifest_data
        )
    ...
    def test_complex_lock_contention_performance(
        self, temp_dir, dispatcher
    ):
        ...
        for i in range(500):
            ...
            needed_locks = random.sample(
                shared_resources, random.randint(3, 5)
            )
        ...
        manifest_path = self.create_manifest(
            temp_dir, "lock_contention.json", manifest_data
        )
    ...
    def test_memory_usage_large_manifests(self, temp_dir, dispatcher):
        ...
        for stage in range(100):
            for task in range(50):
                ...
                task_data = {
                    "agent": agent_id,
                    "priority": "P1",
                    "task": f"Memory test task {task} in stage {stage} with long description "
                    * 10,
                ...
                if stage > 0:
                    task_data["depends_on"] = [
                        f"memory-test-s{stage - 1:02d}-t{task:02d}"
                    ]
        ...
        manifest_path = self.create_manifest(
            temp_dir, "memory_usage.json", manifest_data
        )
        ...
        print(
            f"Memory usage test execution time: {execution_time:.3f}s"
        )

    def test_concurrent_manifest_execution_simulation(
        self, temp_dir, dispatcher
    ):
        ...
        for i in range(5):
            manifest_path = self.create_manifest(
                temp_dir, f"concurrent_{i}.json", manifest_data
            )
        ...
        for manifest_path in manifest_paths:
            ...
            result = test_dispatcher.execute_manifest(
                str(manifest_path)
            )
        ...
        print(
            f"Concurrent simulation total time: {total_execution_time:.3f}s"
        )

    def test_stage_parallelization_potential(
        self, temp_dir, dispatcher
    ):
        ...
        manifest_path = self.create_manifest(
            temp_dir, "parallelization.json", manifest_data
        )
        ...
        print(
            f"Parallelization test execution time: {execution_time:.3f}s"
        )
    ...
    def test_stress_test_extreme_manifest(self, temp_dir, dispatcher):
        ...
        for stage in range(100):
            for task_num in range(100):
                ...
                if stage > 0 and task_num < 50:
                    ...
                    task_data["depends_on"] = [
                        f"stress-s{stage - 1:03d}-t{dependency_task:03d}"
                    ]
        ...
        manifest_path = self.create_manifest(
            temp_dir, "extreme_stress.json", manifest_data
        )
        ...
        print(
            f"Extreme stress test execution time: {execution_time:.3f}s"
        )
