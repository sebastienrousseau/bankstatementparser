...
class TestEuxisDispatchEdgeCases:
    ...
    def test_circular_dependency_detection(self, temp_dir, dispatcher):
        ...
        manifest_path = self.create_manifest(
            temp_dir, "circular_deps.json", manifest_data
        )
    ...
    def test_missing_dependency(self, temp_dir, dispatcher):
        ...
        manifest_path = self.create_manifest(
            temp_dir, "missing_deps.json", manifest_data
        )
    ...
    def test_malformed_manifest_structure(self, temp_dir, dispatcher):
        ...
        for i, manifest_content in enumerate(malformed_manifests):
            ...
            if isinstance(manifest_content, str):
                ...
                try:
                    result = dispatcher.execute_manifest(
                        str(manifest_path)
                    )
                    ...
                    if "dispatches" not in manifest_content:
                        assert result["status"] in [
                            "SUCCESS",
                            "FAILED",
                        ]  # Should handle empty dispatches
    ...
    def test_invalid_stage_numbers(self, temp_dir, dispatcher):
        ...
        manifest_path = self.create_manifest(
            temp_dir, "invalid_stages.json", manifest_data
        )
        dispatcher.execute_manifest(str(manifest_path))
        ...
        assert (
            "large-stage" in executed_agents
        )  # Stage 999 should be last
    ...
    def test_priority_edge_cases(self, temp_dir, dispatcher):
        ...
        manifest_data = {
            ...
            "dispatches": [
                ...
                },
                {
                    "agent": "lowercase",
                    "priority": "p1",
                    "task": "Lowercase priority",
                    "verify_cmd": "echo ok",
                },
                {
                    "agent": "no-priority",
                    "task": "No priority field",
                    "verify_cmd": "echo ok",
                },
                {
                    "agent": "numeric",
        ...
        manifest_path = self.create_manifest(
            temp_dir, "priority_edges.json", manifest_data
        )
    ...
    def test_empty_and_null_fields(self, temp_dir, dispatcher):
        ...
        manifest_path = self.create_manifest(
            temp_dir, "empty_fields.json", manifest_data
        )
        result = dispatcher.execute_manifest(str(manifest_path))
    ...
    def test_unicode_and_special_characters(self, temp_dir, dispatcher):
        ...
        manifest_path = self.create_manifest(
            temp_dir, "unicode_test.json", manifest_data
        )
        result = dispatcher.execute_manifest(str(manifest_path))
    ...
    def test_deep_dependency_chains(self, temp_dir, dispatcher):
        ...
        manifest_path = self.create_manifest(
            temp_dir, "deep_chain.json", manifest_data
        )
        result = dispatcher.execute_manifest(str(manifest_path))
    ...
    def test_many_concurrent_locks(self, temp_dir, dispatcher):
        ...
        manifest_path = self.create_manifest(
            temp_dir, "many_locks.json", manifest_data
        )
        result = dispatcher.execute_manifest(str(manifest_path))
        ...
        greedy_task = next(
            ...
            if task["agent"] == "greedy-task"
        )
        assert len(greedy_task["locks"]) == 100

    def test_stage_failure_with_complex_dependencies(
        ...
        manifest_path = self.create_manifest(
            temp_dir, "complex_failure.json", manifest_data
        )
        result = dispatcher.execute_manifest(str(manifest_path))
    ...
    def test_mixed_mode_compatibility(self, temp_dir, dispatcher):
        ...
        for mode in modes:
            ...
            manifest_path = self.create_manifest(
                temp_dir, f"{mode}_test.json", manifest_data
            )
            result = dispatcher.execute_manifest(str(manifest_path))
    ...
    def test_self_dependency(self, temp_dir, dispatcher):
        ...
        manifest_path = self.create_manifest(
            temp_dir, "self_dep.json", manifest_data
        )
    ...
    def test_duplicate_agent_ids(self, temp_dir, dispatcher):
        ...
        manifest_path = self.create_manifest(
            temp_dir, "duplicate_agents.json", manifest_data
        )
        result = dispatcher.execute_manifest(str(manifest_path))
    ...
    def test_extremely_large_manifest(self, temp_dir, dispatcher):
        ...
        manifest_path = self.create_manifest(
            temp_dir, "large_manifest.json", manifest_data
        )
        result = dispatcher.execute_manifest(str(manifest_path))
    ...
    def test_abort_on_stage_failure_disabled(
        self, temp_dir, dispatcher
    ):
        ...
        manifest_path = self.create_manifest(
            temp_dir, "no_abort.json", manifest_data
        )
        result = dispatcher.execute_manifest(str(manifest_path))
        ...
        )  # Overall failed due to failing task
